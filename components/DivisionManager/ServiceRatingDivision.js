...
const ServiceRatingDivision = ({ navigation, route }) => {
  ...
  useEffect(() => {
    const fetchdata = async () => {
      const res = await authAxios.get(
        Baseurl +
          "api/section-vise-feedback-data/" +
          datas.division_id +
          "/" +
          startdate +
          "/" +
  ...
  const Colonybox = (props) => {
    ...
    return (
      <View>
        <TouchableOpacity
          onPress={() => {
            props.navigation.navigate("ServiceRatingEachPool", {
              name: name,
              id: props.id,
              token: token,
              data: datas,
...
export default ServiceRatingDivision;
